...
export function GameCanvas({ map, playerPos, hazardCells, simulation }: Props) {
  ...
  const viewW =
    dimensions.w > 0 ? dimensions.w / (BASE_CELL_PX * zoom) : map.gridWidth;
  const viewH =
    dimensions.h > 0 ? dimensions.h / (BASE_CELL_PX * zoom) : map.gridHeight;

  const cameraX = playerPos.x + 0.5 - viewW / 2;
  const cameraY = playerPos.y + 0.5 - viewH / 2;

  const activeSafePoints =
    simulation.scenarioExits && simulation.scenarioExits.length > 0
      ? []
      : map.safePoints.filter((sp) => simulation.safePointIds.includes(sp.id));

  const viewBoxStr = `${cameraX} ${cameraY} ${viewW} ${viewH}`;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {simulation.walkableCells &&
          simulation.walkableCells.map((key) => {
        ...
        {/* Blocked Routes */}
        {simulation.blockedCells &&
          simulation.blockedCells.map((key) => {
