...
export function GameHUD({
  ...
  const meta = DISASTER_META[disasterType] || {
    label: disasterType.toUpperCase(),
    Icon: AlertTriangle,
    color: "text-red-500",
  };
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/20 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md p-4 shadow-xl">
      ...
      <div
        className={`absolute -right-10 -top-10 h-32 w-32 rounded-full opacity-10 blur-3xl ${color.replace("text-", "bg-")}`}
      />

      {/* Disaster label */}
      <div
        className={`flex items-center gap-2 font-display font-extrabold text-sm tracking-wide ${color} animate-pulse mb-3`}
      >
      ...
      <div className="flex gap-5 items-center">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-xs font-bold text-ink-500 dark:text-slate-400 flex items-center gap-1.5 uppercase tracking-wider">
              <Heart
                className={`h-3.5 w-3.5 ${hpPct <= 20 ? "text-red-500 animate-ping" : "text-rose-500"}`}
              />
              Kesehatan
            </span>
            <span className="text-xs font-black text-ink-700 dark:text-white">
              {Math.round(hp)}{" "}
              <span className="opacity-50 font-normal">/ {maxHp}</span>
            </span>
          </div>
        ...
        {/* Timer */}
        <div
          className={`flex flex-col items-center justify-center min-w-[70px] transition-transform duration-300 ${
            timerCritical
              ? "scale-110 text-red-600 dark:text-red-400 animate-bounce"
              : "text-ink-800 dark:text-white"
          }`}
        >
          <div className="flex items-center gap-1.5 font-mono font-black text-2xl drop-shadow-sm">
            <Clock
              className={`h-5 w-5 ${timerCritical ? "animate-spin" : ""}`}
