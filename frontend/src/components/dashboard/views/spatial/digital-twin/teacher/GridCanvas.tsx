...
export function GridCanvas({
  ...
  useEffect(() => {
    if (dimensions.w > 0 && !hasFitMap.current) {
      fitMap();
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      pt.x = e.clientX;
      pt.y = e.clientY;
      const worldPt = pt.matrixTransform(
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    if (isPanning) {
      ...
      setCamera((prev) => {
        ...
        return clampCamera(
          { ...prev, x: prev.x - dxWorld, y: prev.y - dyWorld },
          viewW,
          viewH,
        );
      });
  ...
  const viewW =
    dimensions.w > 0 ? dimensions.w / (BASE_CELL_PX * camera.zoom) : gridWidth;
  const viewH =
    dimensions.h > 0 ? dimensions.h / (BASE_CELL_PX * camera.zoom) : gridHeight;
          ...
          <button
            onClick={() =>
              setCamera((s) => {
                ...
                return clampCamera(
                  { ...s, zoom: z },
                  dimensions.w / (BASE_CELL_PX * z),
                  dimensions.h / (BASE_CELL_PX * z),
                );
              })
            }
            className="w-7 h-7 rounded text-sm hover:bg-white dark:hover:bg-slate-600 font-bold transition-colors"
          >
            −
          </button>
          <span className="text-xs font-semibold text-ink-600 dark:text-slate-300 w-12 text-center">
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
            showGrid
              ...
              : "text-ink-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700"
          }`}
        >
        ...
        >
          ...
          {showGrid && (
            <rect
              ...
              height={gridHeight}
              fill={
                camera.zoom < 0.5 ? "url(#gridMajorOnly)" : "url(#gridMajor)"
              }
              pointerEvents="none"
            />
          )}
          ...
          {/* Safe Point Glow */}
          {Array.from(
            new Map(safePoints.map((sp) => [`${sp.x},${sp.y}`, sp])).values(),
          ).map((sp) => (
            <rect
          ...
          {highlightCells &&
            Array.from(highlightCells).map((key) => {
        ...
        {[
          ...
          <div key={type} className="flex items-center gap-2">
            <span
              ...
              className="shadow-sm"
            />
            <span className="text-xs font-medium text-ink-600 dark:text-slate-400">
              {label}
            </span>
          </div>
        ))}
