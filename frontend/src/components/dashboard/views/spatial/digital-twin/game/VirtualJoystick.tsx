    ...
    <div className="flex justify-center select-none" style={{ touchAction: 'none' }}>
      ...
      <div 
        ...
        className={`relative w-36 h-36 rounded-full flex items-center justify-center transition-opacity shadow-inner border-2 border-white/40 dark:border-slate-700/50 backdrop-blur-md ${
          disabled ? 'opacity-50 bg-slate-200/50 dark:bg-slate-800/30 cursor-not-allowed' : 'bg-slate-200/40 dark:bg-slate-800/40'
        }`}
      ...
      >
        ...
        <div 
          className={`absolute w-16 h-16 rounded-full shadow-lg border-b-4 transition-transform ease-out duration-75 ${
            disabled ? 'bg-slate-400 border-slate-500' : 'bg-brand-500 border-brand-700 active:bg-brand-400 active:border-b-2 active:translate-y-0.5'
          }`}
          style={{
