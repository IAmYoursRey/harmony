...
export function GameResult({ phase, player, elapsedSeconds, onExit }: Props) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-6 px-4">
      {/* Icon */}
      <div className={`relative w-32 h-32 rounded-3xl flex items-center justify-center transform transition-all hover:scale-105 shadow-2xl ${
        success ? 'bg-gradient-to-br from-green-400 to-green-600' : 'bg-gradient-to-br from-red-500 to-rose-600'
      }`}>
        {success
          ? <CheckCircle2 className="h-16 w-16 text-white drop-shadow-md" />
          : <XCircle className="h-16 w-16 text-white drop-shadow-md" />
        }
        <div className={`absolute -inset-4 rounded-3xl animate-ping opacity-20 ${
      ...
      <div className="text-center mt-6">
        <h2 className={`font-display text-4xl font-extrabold tracking-tight drop-shadow-sm ${
      ...
      <div className="grid grid-cols-3 gap-3 w-full max-w-sm mt-4">
        {[
          ...
          },
          {
            ...
            color: success ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400',
            bg: success ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20'
          },
          {
            Icon: Route,
            label: 'Jarak',
          ...
          <div key={label} className={`${bg} rounded-2xl p-4 text-center border border-white/50 dark:border-slate-700 shadow-sm transition-transform hover:-translate-y-1`}>
            <Icon className={`h-6 w-6 mx-auto mb-2 ${color}`} />
            <p className={`font-black text-lg ${color}`}>{value}</p>
            <p className="text-[10px] font-bold uppercase tracking-widest text-ink-400 dark:text-slate-500 mt-1">{label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="w-full max-w-sm bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
        <div className="flex justify-between text-xs font-bold uppercase tracking-wider text-ink-500 mb-2">
          ...
          <span className="text-ink-700 dark:text-white">{Math.round(player.hp)} <span className="opacity-50">/ {player.maxHp}</span></span>
        </div>
        ...
        {damageTaken > 0 && (
          <p className="text-xs text-red-500 dark:text-red-400 mt-3 text-center bg-red-50 dark:bg-red-900/20 py-2 rounded-lg font-medium">
            Menerima <strong>{damageTaken} HP</strong> kerusakan dari zona bahaya.
          </p>
        )}
      </div>
