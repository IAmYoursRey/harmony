    ...
    >
      <nav className="section-container flex h-16 items-center justify-between sm:h-20">
        <Link to="/" className="group flex items-center gap-2.5">
          <Logo variant="icon" size={40} className="transition-transform group-hover:scale-105" />
        ...
        <div className="hidden items-center gap-3 md:flex">
          <ThemePicker />
          <button onClick={() => setLocale(locale === 'id' ? 'en' : 'id')} className="p-2 text-slate-500 hover:text-brand-600 transition-colors flex items-center justify-center">
            <span className="text-xs font-bold font-mono">{locale.toUpperCase()}</span>
          </button>
          <button
            onClick={toggle}
        ...
        <div className="flex items-center gap-2 md:hidden">
          <ThemePicker />
          <button onClick={() => setLocale(locale === 'id' ? 'en' : 'id')} className="flex w-full items-center gap-3 rounded-lg px-4 py-3 text-slate-700 hover:bg-slate-50">
            <Globe className="h-5 w-5 text-slate-400" />
            <span>{locale === 'id' ? 'Switch to English' : 'Ganti ke Bahasa Indonesia'}</span>
          </button>
          <button
            onClick={toggle}
