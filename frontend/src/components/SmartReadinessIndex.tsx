...
export type ReadinessCategory =
  "Very Ready" | "Ready" | "Needs Improvement" | "Needs Assistance";
...
export function calculateReadinessIndex(f: ReadinessFactors): ReadinessResult {
  const breakdown = [
    { label: "Knowledge", value: f.knowledge, weight: 0.25, icon: Brain },
    {
      ...
      value: f.decisionAccuracy,
      weight: 0.25,
      icon: Target,
    },
    {
      label: "Evacuation Time",
      value: f.evacuationTime,
      weight: 0.2,
      icon: Clock,
    },
    {
      label: "Learning Consistency",
      value: f.learningConsistency,
      weight: 0.15,
      icon: Activity,
    },
    {
  ...
  return {
    ...
    breakdown: breakdown.map((b) => ({
      ...b,
      weighted: Math.round(b.value * b.weight),
    })),
  };
}
...
export function SmartReadinessIndex({ result }: { result: ReadinessResult }) {
  ...
  const translateCategory = (cat: ReadinessCategory) => {
    switch (cat) {
      case "Very Ready":
        return t("sri.cat.very_ready");
      case "Ready":
        return t("sri.cat.ready");
      case "Needs Improvement":
        return t("sri.cat.needs_imp");
      case "Needs Assistance":
        return t("sri.cat.needs_ast");
      default:
        return cat;
    }
  };
  ...
  const translateFactor = (label: string) => {
    switch (label) {
      case "Knowledge":
        return t("sri.factor.knowledge");
      case "Decision Accuracy":
        return t("sri.factor.decision");
      case "Evacuation Time":
        return t("sri.factor.evac_time");
      case "Learning Consistency":
        return t("sri.factor.consistency");
      case "Improvement":
        return t("sri.factor.improvement");
      default:
        return label;
    }
  };
    ...
    <div className="space-y-6">
        ...
        <div className="relative flex flex-col items-center gap-6 sm:flex-row sm:justify-between">
          ...
          <div className="relative flex h-44 w-44 items-center justify-center">
            ...
            <div className="absolute flex flex-col items-center">
              ...
              <span className="text-xs font-medium text-ink-500 dark:text-slate-400">
                {t("sri.out_of")}
              </span>
            </div>
          </div>
          ...
          <div className="flex-1 text-center sm:text-left">
            ...
            <div className="mt-3 flex flex-wrap justify-center gap-3 sm:justify-start">
              <div className="rounded-xl bg-brand-50/60 px-4 py-2.5 dark:bg-slate-800/60">
                <p className="text-[10px] uppercase tracking-wider text-ink-400 dark:text-slate-500">
                  {t("sri.prev")}
                </p>
              ...
              <div className="rounded-xl bg-brand-50 px-4 py-2.5 dark:bg-brand-950/30">
                <p className="text-[10px] uppercase tracking-wider text-brand-600">
                  {t("sri.improve")}
                </p>
            ...
            <span
              className={`mt-3 inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-sm font-bold ${style.bg} ${style.text}`}
            >
      ...
      <div className="glass rounded-2xl p-5 dark:bg-slate-900/60">
        ...
        <div className="mt-4 rounded-xl bg-brand-50/60 p-4 text-xs text-ink-600 dark:bg-slate-800/60 dark:text-slate-300">
          ...
          <p className="mt-1">{t("sri.formula_desc")}</p>
        </div>
      </div>
      ...
      <div className="glass rounded-2xl p-5 dark:bg-slate-900/60">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[
            {
              label: t("sri.cat.very_ready"),
              range: "85-100",
              color: "bg-brand-600",
              active: result.category === "Very Ready",
            },
            {
              label: t("sri.cat.ready"),
              range: "70-84",
              color: "bg-brand-500",
              active: result.category === "Ready",
            },
            {
              label: t("sri.cat.needs_imp"),
              range: "50-69",
              color: "bg-brand-400",
              active: result.category === "Needs Improvement",
            },
            {
              label: t("sri.cat.needs_ast"),
              range: "0-49",
              color: "bg-brand-300",
              active: result.category === "Needs Assistance",
            },
          ].map((c) => (
            <div
              key={c.label}
            ...
            >
              <span
                className={`inline-block h-3 w-3 rounded-full ${c.color}`}
              />
              <p className="mt-2 text-sm font-bold text-ink-900 dark:text-white">
                {c.label}
              </p>
