...
import {
  Bot, SendHorizonal, BookOpen, Zap, Waves, Flame, Wind, AlertTriangle,
  Trophy, Star, BarChart3, ChevronRight, RotateCcw, CheckCircle, XCircle,
  Loader2, PenLine, MessageSquare, Award, TrendingUp, Lock
} from 'lucide-react';
import { useAuth } from '@/context/AuthContext';
import { getProfile, buildAISummary, recordQuizSession, createInitialTopicScore } from '@/data/userProfiles';
import { askChatbotAI, generateQuizQuestions, evaluateQuizAnswers } from '@/services/geminiService';
import type { QuizQuestion, QuizEvaluation, QuizDifficulty } from '@/services/geminiService';
...
export function AILearningView() {
  ...
  const submitQuiz = async () => {
    ...
    if (currentUser) {
      ...
      const points = Math.round(totalScore * 0.5);
      const result = recordQuizSession(currentUser.id, topicLabel, totalScore, weak, strong, points);
