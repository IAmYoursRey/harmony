...
import { getAllProfiles, updateProfile } from '@/data/userProfiles';
...
export default function TeacherDashboardView() {
  ...
  const [newGrade, setNewGrade] = useState<'10'|'11'|'12'>('10');
          ...
          >
            <form onSubmit={handleAddClass} className="glass rounded-2xl p-5 border-l-4 border-l-brand-500 flex flex-col sm:flex-row items-end gap-4">
              <div className="flex-1 space-y-1">
                ...
                <select value={newGrade} onChange={e => setNewGrade(e.target.value as '10'|'11'|'12')}
                  className="w-full rounded-xl border border-brand-100 bg-white/70 py-2.5 px-3 text-sm outline-none focus:ring-2 focus:ring-brand-100 dark:border-slate-700 dark:bg-slate-800/70 dark:text-white">
              ...
              <div className="flex-1 space-y-1">
                <label className="text-xs font-bold text-ink-700 dark:text-slate-300">Ruang/No. Kelas (Manual)</label>
                <input type="text" value={newSection} onChange={e => setNewSection(e.target.value)} placeholder="Contoh: 1 atau IPA 2" required
                  className="w-full rounded-xl border border-brand-100 bg-white/70 py-2.5 px-4 text-sm outline-none focus:ring-2 focus:ring-brand-100 dark:border-slate-700 dark:bg-slate-800/70 dark:text-white" />
              </div>
