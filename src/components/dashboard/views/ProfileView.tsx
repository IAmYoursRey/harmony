      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="flex flex-col">
          ...
          <div className="space-y-3">
            ...
            <div className="flex w-full items-center gap-3 rounded-xl border border-brand-50 p-3 transition-colors dark:border-slate-800">
              <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-brand-100 text-brand-600 dark:bg-slate-800 dark:text-brand-400"><Globe className="h-4 w-4" /></span>
              <div className="flex-1">
                <p className="text-sm font-semibold text-ink-900 dark:text-white">{t('profile.language')}</p>
                <p className="text-xs text-ink-500 dark:text-slate-400">{t('profile.language.desc')}</p>
              </div>
              <div className="flex items-center gap-1 rounded-lg bg-brand-50 p-1 dark:bg-slate-800">
                <button
                  onClick={() => setLang('id')}
                  className={`rounded-md px-2 py-1 text-xs font-semibold transition-all ${lang === 'id' ? 'bg-brand-600 text-white shadow-sm' : 'text-ink-500 hover:bg-white/50 dark:text-slate-400 dark:hover:bg-slate-700'}`}
                >
                  🇮🇩 ID
                </button>
                <button
                  onClick={() => setLang('en')}
                  className={`rounded-md px-2 py-1 text-xs font-semibold transition-all ${lang === 'en' ? 'bg-brand-600 text-white shadow-sm' : 'text-ink-500 hover:bg-white/50 dark:text-slate-400 dark:hover:bg-slate-700'}`}
                >
                  🇬🇧 EN
                </button>
              </div>
