...
export function SchoolLocationSelector() {
  ...
  const regencies = findSchoolsByProvince(provinceId).map((r) => ({ id: r.id, name: r.name }));
  const schools = findSchoolsByRegency(provinceId, regencyId).map((s) => ({ id: s.id, name: s.name }));

  // Retrieve current active school selection based on active schoolId
  const selectedSchool = useMemo(() => {
    if (!schoolId) return undefined;
    for (const prov of schoolData) {
      for (const reg of prov.regencies) {
        const sch = reg.schools.find((s) => s.id === schoolId);
        if (sch) return sch;
      }
    }
    return undefined;
  ...
  const selectedSchoolLocationInfo = useMemo(() => {
    ...
    for (const prov of schoolData) {
      for (const reg of prov.regencies) {
        const sch = reg.schools.find((s) => s.id === selectedSchool.id);
        if (sch) {
          return { provinceId: prov.id, regencyId: reg.id };
        }
      }
    }
    return null;
  ...
  const searchResults = useMemo(() => {
    ...
    return searchSchools(globalSearch, 10);
  }, [globalSearch]);

  // Calculate nearest schools if location is detected
  const nearestSchools = useMemo(() => {
    if (!location) return [];
    const filter = levelFilter === 'ALL' ? undefined : [levelFilter as School['level']];
    return findNearestSchools(location.lat, location.lng, 15, filter);
  }, [location, levelFilter]);

  const handleConfirm = () => {
    if (!selectedSchool || !selectedSchoolLocationInfo) return;
    setSelection(selectedSchool, selectedSchoolLocationInfo.provinceId, selectedSchoolLocationInfo.regencyId);
    setConfirmed(true);
  ...
  const handleNearSelect = (sch, provId, regId) => {
        ...
        <div className="glass rounded-2xl p-5 dark:bg-slate-900/60">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <SearchableDropdown
              ...
              icon={MapPin}
              value={provinceId}
              options={schoolData.map((p) => ({ id: p.id, name: p.name }))}
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {nearestSchools.map((item) => {
                    ...
                    return (
                      <button
                        key={item.school.id}
                        onClick={() => handleNearSelect(item.school, item.provinceId, item.regencyId)}
            ...
            <div className="glass space-y-5 rounded-2xl p-6 dark:bg-slate-900/60">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-ink-500 dark:text-slate-400">
                    <span className="flex items-center gap-1">
                      ...
                      {selectedSchoolLocationInfo ? getProvinceName(selectedSchoolLocationInfo.provinceId) : ''}
                    </span>
                    <span>·</span>
                    <span>{selectedSchoolLocationInfo ? getRegencyName(selectedSchoolLocationInfo.provinceId, selectedSchoolLocationInfo.regencyId) : ''}</span>
                    <span>·</span>
                    <span className="flex items-center gap-1">
              ...
              <div>
                ...
                <div className="grid gap-3 sm:grid-cols-5">
                  {hazardIcons.map((h) => {
                    const value = selectedSchool[h.key];
                    return (
