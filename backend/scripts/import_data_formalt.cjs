...
async function run() {
  ...
  if (!fs.existsSync(excelPath)) {
    ...
    console.error(
      `Silakan pastikan file 'data_formalt.xlsx' sudah diletakkan di root direktori GeoSense sebelum menjalankan script ini.`,
    );
    process.exit(1);
  }
  ...
  console.log(
    "Catatan Risiko Bencana (Enrichment): Backend GeoSense (routes/schools.js) secara otomatis memperkaya (enrich) data sekolah ini dengan profil risiko spesifik dari BNPB IRBI berdasarkan 'Regency' (Kabupaten/Kota) pada saat runtime (tanpa membebani memori).",
  );
  console.log("=========================================");
}
